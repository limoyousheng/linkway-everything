...
| 方式 | 适合场景 | 怎么做 |
|---|---|---|
| **离线（推荐）** | 没网、跨电脑、纯本地 | 浏览器直接打开 `AI平台导航.html`（或 `file://` 双击）。Tailwind + SortableJS 已本地化（`tailwindcss.js` / `sortable.min.js`），不需要联网拉样式和拖拽库。 |
| **在线 + state-server** | 要"自动抓图标"和"自动抓描述"功能 | 双击 `open-ai-platform-edge.cmd` 拉起 8765（静态）+ 9001（抓取代理）。 |
